...
kgen3 = Parser(plant=plant).AddModelFromFile(robot_urdf,"kinova_gen3")

# Fix the base of the manipulator to the world
plant.WeldFrames(plant.world_frame(),plant.GetFrameByName("base_link",kgen3))
...
builder.Connect(                              # First controller output is torques to the arm
        ...
        plant.get_actuation_input_port(kgen3))
...
plant.SetPositions(plant_context, kgen3, np.array([0.1,0,0,np.pi/2,-0.1,0,0]))   # Manipulator arm
plant.SetVelocities(plant_context, kgen3, np.zeros(7))

# Run the simulation
simulator.Initialize()
simulator.AdvanceTo(3.00)
